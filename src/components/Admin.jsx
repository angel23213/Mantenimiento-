...
// Tu Contraseña de respaldo para modo Offline (desarrollo local sin Supabase)
...
export default function Admin() {
  ...
  // Un solo useEffect para inicializar todo limpiamente
  useEffect(() => {
    if (!supabase) {
    ...
    // Comprobar de forma segura si ya existe sesión activa
    const initializeAuthAndData = async () => {
      try {
        ...
        // Cargar los productos tanto si está logueado como si no
        await fetchProducts();
      }
    };
  ...
  const handleLogin = async (e) => {
    ...
    if (!supabase) {
      // Modo offline/desarrollo sin variables .env (usa credenciales locales)
    ...
    // Iniciar sesión real en Supabase usando el EMAIL y la CONTRASEÑA ingresados
    const { data, error } = await supabase.auth.signInWithPassword({
      email: email, // <-- Ahora dinámico
      password: pw,
    });
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 space-y-12">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col items-center gap-1 flex-shrink-0">
                        ...
                        >
                          <span
                            className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${item.available ? "translate-x-6" : "translate-x-0.5"}`}
                          />
                        </button>
                        <span
        ...
        <div className="bg-char-800 border border-char-600 rounded-xl p-5 flex gap-3">
          <svg
            ...
            stroke="currentColor"
            strokeWidth={2}
            className="text-lemon-400 flex-shrink-0 mt-0.5"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
          ...
          <p className="font-body text-sm text-gray-400">
            <span className="text-white font-500">Conexión activa:</span> El
            guardado se realiza instantáneamente en segundo plano en Supabase.
            Los cambios se sincronizan en tiempo real en todas las ventanas
            activas.
          </p>
        </div>
      </div>
